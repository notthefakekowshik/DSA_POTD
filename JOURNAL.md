...
| --- | --- | --- | --- | --- | --- |
| 33 | 2026-03-27 | Relative Ranks | Easy | Max-Heap | Max-heap of (score, index) pairs extracts athletes in rank order; mirrors the "extract-max N times" pattern from Last Stone Weight. |
| 34 | 2026-03-27 | Design Twitter | Medium | Heap + Hash Map | `getNewsFeed` is Merge K Sorted Lists in disguise — heap entry carries a pointer to the user's next older tweet, pulling only what's needed. |
| 35 | 2026-03-27 | Find K Pairs with Smallest Sums | Medium | Min-Heap (K-way merge) | Seed heap with (nums1[i], nums2[0]) for each row; advance only j per extraction. Identical pointer mechanics to Merge K Sorted Lists and Design Twitter. |
| 36 | 2026-03-27 | IPO | Hard | Two Heaps (Greedy) | Capital min-heap enables dynamic eligibility tracking; profit max-heap provides greedy selection. Always pick highest profit among affordable projects — proven optimal by exchange argument. |
| 37 | 2026-03-27 | Sliding Window Median | Hard | Two TreeSets | TreeSet provides O(log K) removal unlike PriorityQueue's O(N) remove(). Handle duplicates via (value, index) comparator. Cast to long before addition to prevent overflow: `((long)a + (long)b) / 2.0`. |
| 38 | 2026-03-27 | Kth Smallest Element in a Sorted Matrix | Medium | Min-Heap (K-way merge) | Each row is a sorted list — identical to Merge K Sorted Lists but stop at Kth element. O(K log N) heap vs O(N log(max-min)) binary search; heap wins when K << N². |
| 39 | 2026-03-27 | Minimum Cost to Connect Sticks | Medium | Min-Heap (Huffman) | Always combine two smallest sticks to minimize total cost. Identical to Huffman coding tree construction — greedy correctness proven by exchange argument comparing (a+b) vs (a+c) costs. |
| 40 | 2026-03-27 | Meeting Rooms II | Medium | Min-Heap (Intervals) | Min-heap tracks end times of ongoing meetings. Heap size at any point = concurrent meetings = rooms needed. Sweep line algorithm: sort starts, process chronologically with heap. |
| 41 | 2026-03-27 | Reorganize String | Medium | Max-Heap (Greedy) | Always pick most frequent character (different from previous) to prevent accumulation. Impossible if max_freq > (n+1)/2. Proven optimal: placing max_freq at even indices guarantees validity. |
| 42 | 2026-03-27 | Ugly Number II | Medium | Three-Pointer DP | Merge 3 infinite sorted lists (multiples of 2, 3, 5). Three pointers track next candidate from each list. Advance ALL pointers producing minimum to handle duplicates (e.g., 2×3 = 3×2). O(N) beats heap's O(N log N). |
| 43 | 2026-03-27 | Trapping Rain Water II | Hard | Min-Heap BFS | Process from boundary inward using min-heap ensures we know water level when reaching each cell. Similar to Dijkstra: water at cell = max(0, min_boundary_height - cell_height). 2D extension of two-pointer 1D approach. |
